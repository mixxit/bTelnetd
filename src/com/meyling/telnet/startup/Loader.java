...
import com.meyling.telnet.shell.PwtsShell;
...
/**
  ...
  (Very small adaptations by Michael Meyling.)
 */
...
public class Loader  { 
  ...
  static {
    ...
    String ignoreTCLProp = System.getProperty("log4j.ignoreTCL", null);
  ...
  */
  static public URL getResource(String resource) {
    ...
    if(!java1) {
      classLoader = getTCL();
      if(classLoader != null) {
        trace.debug("Trying to find ["+resource+"] using context classloader "
             +classLoader+".");
    ...
    if(classLoader != null) {
      trace.debug("Trying to find ["+resource+"] using "+classLoader
               +" class loader.");
      url = classLoader.getResource(resource);
      if(url != null) {
        return url;
